...
const PremiumPageUser = () => {
  ...
  const [premiumPlan, setPremiumPlan] = useState(null); // State for premium plan
  const [showModal, setShowModal] = useState(false); // State to control modal visibility
  const [selectedPremium, setSelectedPremium] = useState(null); // State for selected premium plan
  ...
  useEffect(() => {
    ...
    const fetchAlbumsAndPlaylists = async () => {
      try {
        ...
        // Fetch premium plans
        const premiumResponse = await axios.get('http://localhost:8070/payment/all');
        const userPremiumPlan = premiumResponse.data.find(plan => plan.user.userId === userId);
        setPremiumPlan(userPremiumPlan);
      } catch (error) {
  ...
  const calculateRemainingDays = (transactionDate, premiumvalidity) => {
    console.log('Transaction Date:', transactionDate);
    console.log('Premium Validity:', premiumvalidity);

    const transactionDateObj = new Date(transactionDate);
    let endDateObj;

    if (typeof premiumvalidity !== 'string') {
        console.error('Invalid validity format:', premiumvalidity);
        return 0;
    }

    const validityMatch = premiumvalidity.match(/^(\d+)\s*(year|years|month|months|week|weeks)$/i);
    console.log('Validity Match:', validityMatch);

    if (validityMatch && validityMatch[2]) {
        const amount = parseInt(validityMatch[1], 10);
        const unit = validityMatch[2].toLowerCase();

        endDateObj = new Date(transactionDateObj);

        if (unit === 'year' || unit === 'years') {
            endDateObj.setFullYear(endDateObj.getFullYear() + amount);
        } else if (unit === 'month' || unit === 'months') {
            endDateObj.setMonth(endDateObj.getMonth() + amount);
        } else if (unit === 'week' || unit === 'weeks') {
            endDateObj.setDate(endDateObj.getDate() + (amount * 7)); // Adding weeks as days
        }
    } else {
        console.warn('Invalid validity format:', premiumvalidity);
        return 0; // Return 0 if validity format is unknown
    }
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
                    {filteredAlbums.map((album) => (
                      <div key={album.albumId} className="relative bg-gray-800 rounded-lg overflow-hidden shadow-lg">
                        ...
                        />
                        <button
                          onClick={() => navigate('/viewsongspremium', { state: { album } })}
      ...
      {showModal && selectedPremium && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-75 z-20">
          <div className="bg-gray-800 text-white rounded-lg p-6 max-w-lg mx-4">
            <h3 className="text-2xl font-semibold mb-4">Premium Plan Details</h3>
            <p className="mb-4"><strong>Premium Name:</strong> {selectedPremium.premium.premiumName}</p>
            <p className="mb-4"><strong>Transaction On</strong> {selectedPremium.transactionDate}</p>
            <p className="mb-4"><strong>Remaining Days:</strong> {calculateRemainingDays(selectedPremium.transactionDate, selectedPremium.premium.premiumvalidity)} days</p>
            <button
              onClick={closeModal}
...
export default PremiumPageUser;
